VPL_PROFORMA_INTEGRATION_VERSION="1.0.2"
...
if [ -f "ProformaFormatter-$VPL_PROFORMA_INTEGRATION_VERSION-fat-jar-with-dependencies.jar" ]; then
    ...
    if [ -n "$task_file" ]; then
        ...
        echo "java -jar ProformaFormatter-$VPL_PROFORMA_INTEGRATION_VERSION-fat-jar-with-dependencies.jar $file_list $task_file_name" >> vpl_execution
    else
    ...
    echo "Missing file ProformaFormatter-$VPL_PROFORMA_INTEGRATION_VERSION-fat-jar-with-dependencies.jar. Make sure the mentioned file is included in the 'Execution files' list"
fi
